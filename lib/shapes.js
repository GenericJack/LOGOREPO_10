...
class Square {
  ...
  render() {
    ...
    const escapedColor = this.color.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    const svg = `<rect x="10" y="10" width="100" height="100" fill="${escapedColor}" />`;
    return svg;
  }
}
...
class SVG {
  ...
  setShapeText(text, color) {
    this.shapeText = `<text x="50" y="100" fill="${color}">${text}</text>`;
  }
